...
class WorkerThread(QThread):
    ...
    def remove_duplicates(self):
        ...
        unique_rows = [rows[0]]  # Keep the header

        for row in rows[1:]:
            is_duplicate = False
            for unique_row in unique_rows[1:]:
                # Check if the first two columns are different but all other columns are the same
                if (row[0] == unique_row[0] and row[1] == unique_row[1]) or row[2:] != unique_row[2:]:
                    continue
                else:
                    is_duplicate = True
                    break
        ...
        # Save the unique rows back to a new CSV file or overwrite the existing one
        with open(inp, 'w', encoding='ISO-8859-1', newline='') as file:
